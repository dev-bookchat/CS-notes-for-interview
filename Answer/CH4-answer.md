...
만약 서비스를 구축한다면 가장 먼저 신경써야 합니다. 
ERD는 시스템 요구사항을 기반으로 작성되며, 이 ERD를 기반으로 데이터베이스를 구축합니다. 
데이터베이스를 구축한 후에도 디버깅 또는 비즈니스 프로세스 재설계가 필요한 경우 설계도 역할을 담당하기도 합니다. 
그러므로 ERD는 관계형 구조로 표현할 수 있는 데이터를 구성하는 데에는 유용하다는 장점이 있지만 
비정형 데이터는 충분히 표현할 수 없는 단점도 있습니다. 

### Q2. 정규화 과정에 대해 설명하고 종류를 생각나는대로 말해보세요.
정규화 과정은 릴레이션 간의 잘못된 종속 관계로 인해 데이터베이스 이상 현상이 일어나서 이를 해결하거나, 저장 공간을 효율적으로 사용하기 위해 
릴레이션을 여러개로 분리하는 과정입니다. 
정규화 과정은 정규형 원칙을 기반으로 정규형을 만들어가는 과정이며, 정규화된 정도는 정규형으로 표현합니다.
기본 정규형인 제1정규형, 제2정규형, 제3정규형, 보이스/코드 정규형이 있고 고급 정규형인 제4정규형, 제5정규형이 있습니다.
...
이행적 함수 종속이란 A -> B와 B -> C 가 존재하면 논리적으로 A -> C가 성립하는데 
이 때 집합 C가 집합 A에 이행적으로 함수 종속이 되었다고 합니다. 

### Q5. 보이스/코드 정규형에 대해 설명해보세요.
보이스/코드 정규형(BCNF)는 제3정규형이고, 
결정자가 후보키가 아닌 함수 종속 관계를 제거하여 
릴레이션의 함수 종속 관계에서 
모든 결정자가 후보키인 상태를 말합니다.
...
트랜젝션은 데이터베이스에서 하나의 논리적 기능을 수행하기 위한 작업의 단위를 말하며
데이터베이스에 접근하는 방법은 쿼리이므로, 즉 여러 개의 쿼리들을 하나로 묶는 단위를 말합니다. 
이에 대한 특징은 원자성, 일관성, 독립성, 지속성이 있으며 이를 한꺼번에 ACID 특징이라고 합니다.

### Q2. 원자성에 대해 설명해 보세요.
원자성은 트랜잭션과 관련된 일이 모두 수행되었거나 되지 않았거나를 보장하는 특징입니다. 
예를 들어 트랜잭션을 커밋했는데, 문제가 발생하여 롤백하는 경우 그 이후에 모두 수행되지 않음을 보장하는 것을 뜻합니다.

### Q3. 트랜잭션 전파는 무엇입니까?
트랜잭션을 수행할 때 커넥션 단위로 수행하기 때문에 커넥션 객체를 넘겨서 수행해야 합니다.
하지만 이를 매번 넘겨주기가 어렵거나 귀찮으므로, 커넥션을 넘기지 않고 
여러 트랜잭션 관련 메서드의 호출을 하나의 트랜젝션에 묶이도록 하는 것을 트랜잭션 전파라고 합니다. 

### Q4. 격리성에 대해 얘기해보고 왜 트랜잭션을 격리를 해야하는지 설명해보세요.
격리성은 트랜잭션 수행 시 서로 끼어들지 못하는 것을 말합니다. 
복수의 병렬 트랜잭션을 실행시킬 때 순서대로가 아닌 성능이 좋은 순서대로 실행을 하지만
마치 순차적으로 실행되는 것처럼 작동시키기 위해 트랜잭션을 격리합니다.
그러기 우해 격리성은 트랜 잭션을 여러 개의 격리 수준을 나누어 격리성을 보장합니다. 
...
- 팬텀 리드
  ...
  A가 어떤 결과를 조회하는 쿼리를 보냈는데 그 결과를 가져오기 전에 B가 그 결과값이 헤당되는 값을 하나 추가하게 된다면
  초기의 조회값이 A에게 가지 않고 B가 입력한 값 하나가 더 추가된 결과가 A에게 전달됩니다.
  결과적으로 A가 조회를 시작할 때와 조회 결과를 받을때 검색된 값이 다른 경우를 뜻합니다.
  
- 반복 가능하지 않은 조회
  반복 가능하지 않은 조회는 한 트랜잭션 내의 같은 행에 두 번 이상 조회가 발생했는데, 그 값이 다른 경우를 가리킵니다. 
  조회를 반복했는데, 그 값이 다르니 반복이 가능하지 않은 거죠.
  예를 들어 사용자 1이 자신의 나이를 30으로 썼는데, 사용자 2가 사용자 1의 나이를 20으로 바꿔서 커밋해버리면 
  그 이후 사용자 1은 자신의 나이를 20으로 읽게 됩니다.
  
  팬텀 리드와 반복 가능하지 않은 조회가 다른 점은 값이 변화하느냐와 다른 값도 선택하느냐의 차이입니다.
  
- 더티 리드
  더티 리드는 반복 가능하지 않은 조회와 유사한데 
  한 트랜잭션이 실행 중일 때 다른 트랜잭션에 의해 수정되었지만 '아직 커밋되지 않은' 행의 데이터를 읽을 수 있을 떄를 말합니다. 
  사용자 1이 자신의 나이를 30으로 썼는데, 사용자 2가 사용자 1의 나이를 20으로 바꿨지만 커밋하지 않았는데도  
  그 이후 사용자 1은 자신의 나이를 20으로 읽게 되는 경우를 말합니다.
  
### Q6. 격리 수준을 나누어 설명해보세요.
-SERIALIZABLE(순차적인)
  말 그대로 트랜잭션을 순차적으로 진행시킵니다. 
  여러 트랜잭션이 동시에 같은 행에 접근할 수 없습니다.
  매우 엄격한 수준으로 해당 행을 격리 시키기 떄문에 교착 상태가 일어날 확률도 가장 높고 가장 성능이 떨어집니다.
  
-REPEATABLE_READ(반복가능한 조회)
하나의 트랜잭션이 수정한 행을 다른 트랜잭션이 수정할 수 없게 막지만 
새로운 행을 추가하는 것은 막지 않습니다. 

-READ_COMMITTED(커밋된 트랜잭션 조회)
가장 많이 사용되는 격리 수준이며, 여러 데이터베이스에서 기본값으로 설저오디어 있습니다.
다른 트랜잭션이 커밋하지 않은 정보는 읽을 수 없습니다. 
하지만 어떤 트랜잭션이 접근한 행을 다른 트랜잭션이 수정할 수 있어 반복 조회시 다른 내용을 발견할 수 있습니다. 

-READ_UNCOMMITTED(커밋되지는 않았지만 변경된 트랜잭션 조회)
가장 낮은 격리 수준으로, 가장 빠릅니다. 
데이터 무결성을 위해 되도록잉면 사용하지 않는 것이 좋지만, 거대한 양의 데이터를 
어림잡아 집계하는 데 사용하기 좋습니다. 
